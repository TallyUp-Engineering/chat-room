...
| 16 | Nothing reaches PyPI that has not been installed back out of a package index and run. Publishing is irreversible and yanking is deliberately manual — Warehouse's yank route is UI-only and behind a re-authentication challenge — so the last check has to come before the irreversible step. | `.github/workflows/package.yml` (`build` → `verify` → `publish`) |
