...
| 18 | Work started on someone's machine is announced, attributable, and refusable. Background warming says it started, runs one per room behind a lock it claims itself, logs where a person can read it, and stops when the operator says so. | `test_a_warm_is_offered_once_and_only_when_asked_for`, `test_only_one_warmer_can_hold_a_room`, `test_the_operator_can_turn_background_warming_off` |
| 15 | Every pair that could collide is asked about, and a scan reports its own coverage regardless. There is no probe cap: whether two commits conflict is a fixed fact, so the answer is memoised against the pair of commits and a full sweep costs once. Coverage is still reported, because a probe can still fail. | `test_merge_readiness_asks_about_every_pair_that_shares_a_file`, `test_a_merge_answer_is_remembered_against_the_commits_it_compared`, `test_a_truncated_conflict_scan_says_so` |
